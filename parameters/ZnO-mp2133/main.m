...
R_List = [
    0.66666667 0.33333333 0.5005917;  % Zn
    3.33333333e-01 6.66666667e-01 5.91700000e-04;  % Zn
    0.66666667 0.33333333 0.8797183 ;  % O
    0.33333333 0.66666667 0.3797183 ;  % O
...
filename = sprintf('ZnO_alpha=%g, beta=%g, RMax=%g', alpha, beta, RMax);  % 项目名称

if isempty(gcp('nocreate'))  % 预启动并行运算池
    parpool;
end

H = HMatrixFast(phi, R_List, R_L_List, potential); 
S = SMatrixFast(phi, R_List, R_L_List);  % 若假设原子轨道完美正交，则可令 S = eye(length(phi(:, 1)));
